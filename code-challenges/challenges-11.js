...
function square(arr) {
    ...
    let squArr = arr.map(item => item * item)
    return squArr;
}
...
function fullName(arr) {
    // write your code here
    let fullNameArr = arr.map(fullNamme => fullNamme.firstName + ' ' + fullNamme.lastName);
    return fullNameArr;
}
...
function gradesAvg(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        arr[i].gradsList.map(grade => {
            return avgGrade = avgGrade + grade / arr[i].gradsList.length
        })
        arr[i].avg = avgGrade;
        avgGrade = 0;
    }
